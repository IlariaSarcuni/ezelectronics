...
class ProductRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            "/",
            body("model").notEmpty().withMessage("Product model must be provided and cannot be empty."),
            body("category").notEmpty().isIn(['Smartphone', 'Laptop', 'Appliance']).withMessage("Product category must be one of Smartphone, Laptop or Appliance."),
            body("quantity").isInt({ min: 1 }).withMessage("Product quantity must be an integer greater than 0."),
            body("sellingPrice").isFloat({ min: 1 }).withMessage("Product selling price must be a float greater than 0."),
            body("arrivalDate").optional().isString().notEmpty().withMessage("Arrival date must be a valid date."),
        ...
        this.router.patch(
            "/:model",
            param("model").notEmpty().withMessage("Product model must be provided and cannot be empty."),
            body("quantity").isInt({ min: 1 }).withMessage("Product quantity must be an integer greater than 0."),
            body("changeDate").optional({ checkFalsy: true }).isDate().withMessage("Change date must be a valid date."),
            this.authenticator.isAdminOrManager,
            this.errorHandler.validateRequest,
        ...
        this.router.patch(
            ...
            param("model").notEmpty().withMessage("Product model must be provided and cannot be empty."),
            body("quantity").isInt({ min: 1 }).withMessage("Product quantity must be an integer greater than 0."),
            body("sellingDate").optional({ checkFalsy: true }).isDate().withMessage("Selling date must be a valid date."),
            this.authenticator.isAdminOrManager,
            this.errorHandler.validateRequest,
        ...
        this.router.get(
            ...
            query('category').optional().isIn(['Smartphone', 'Laptop', 'Appliance']).withMessage('Category must be one of "Smartphone", "Laptop", "Appliance"'),
            this.authenticator.isAdminOrManager,
            this.errorHandler.validateRequest,
